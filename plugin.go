...
func (ka *KeyAuth) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
    // Create a response writer wrapper to capture and modify headers before they are sent
    wrappedRW := &responseWriterWrapper{ResponseWriter: rw, ignoreHeaders: []string{}}
    ...
    if ka.authenticationHeader {
        if contains(req.Header.Get(ka.authenticationHeaderName), ka.keys) {
            ...
            if ka.removeHeadersOnSuccess {
                ...
                wrappedRW.ignoreHeaders = append(wrappedRW.ignoreHeaders, ka.authenticationHeaderName)
            }
    ...
    if ka.bearerHeader {
        if bearer(req.Header.Get(ka.bearerHeaderName), ka.keys) {
            ...
            if ka.removeHeadersOnSuccess {
                ...
                wrappedRW.ignoreHeaders = append(wrappedRW.ignoreHeaders, ka.bearerHeaderName)
            }
            ka.next.ServeHTTP(wrappedRW, req)
            return
        }
...
// responseWriterWrapper is a custom http.ResponseWriter that allows us to modify headers before they are sent
type responseWriterWrapper struct {
    http.ResponseWriter
    ignoreHeaders []string
}

// WriteHeader is overridden to modify headers before the response is written
